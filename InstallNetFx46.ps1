...
if($netFxKey -and $netFxKey.Release -ge 394806) {
    "$(Get-Date): The machine already has NetFx 4.6.2 or later version installed." | Tee-Object -FilePath $logFile -Append
    exit 0
}

# Download the latest NetFx46
$setupFileSourceUri = "https://download.microsoft.com/download/D/5/C/D5C98AB0-35CC-45D9-9BA5-B18256BA2AE6/NDP462-KB3151802-Web.exe"
$setupFileLocalPath = Join-Path $env:TEMP -ChildPath "NDP462-KB3151802-Web.exe"

"$(Get-Date): Start to download NetFx 4.6.2 to $setupFileLocalPath." | Tee-Object -FilePath $logFile -Append
...
if(!(Test-Path $setupFileLocalPath))
{
    "$(Get-Date): Failed to download NetFx 4.6.2 setup package." | Tee-Object -FilePath $logFile -Append
    exit -1
}
...
"$(Get-Date): Start to install NetFx 4.6.2" | Tee-Object -FilePath $logFile -Append
